...
var Artists = function (_React$Component) {
	...
	_createClass(Artists, [{
		...
		value: function componentDidMount() {
			...
			//Get Artists
			_Queries2.default.postRequest({ query: _Queries2.default.artist.getAll }, function (artists) {
				console.log(artists);
				_this2.setState({
		...
		value: function render() {
			var artistList = this.state.artists.map(function (artist) {
				return React.createElement(_Square2.default, {
					key: artist._id,
					title: artist.name,
					path: artist.path
				});
