...
export default class Article extends React.Component {
	...
	handleClick() {
		if (!this.state.open) {
			...
			document.body.classList.add("noscroll"); //Disable scrolling beneath open article

			//Open 50ms after click so we can get the start position before animating
			this.setState(
				{
					startPos: pos
				},
	...
	close() {
		...
		if (this.state.startPos) {
			...
			// Article has no start position so was in URL - close without animating
			this.setState({
				startPos: null,
				canHover: true
			});
	...
	render() {
		return (
			...
			>
				...
				>
					<div style={this.state.startPos && { display: "block" }}>
						...
						<div className="article-date">
							{this.props.article.date.toDateString()}
						</div>
						<div
				...
				<div
					className={`article-title valign-wrapper ${
